...
abstract class AbstractIndexTests(store: DataStore) : BaseTest(store) {
    ...
    fun `id is correctly increased after insert`() = runDatabaseTest {
    ...
    fun `index is correctly created after insert`() = runDatabaseTest {
        val collection = db.getObjectCollection<TestUser>("test")
        collection.insert(TestUser.Mario)
    ...
    fun `index is correctly created before insert`() = runDatabaseTest {
        val collection = db.getObjectCollection<TestUser>("test")
        collection.createIndex("name")
        collection.insert(TestUser.Mario)
    ...
    fun `index is correctly created after insert and update`() = runDatabaseTest {
        val collection = db.getObjectCollection<TestUser>("test")
        collection.insert(TestUser.Mario)
        collection.createIndex("name")
