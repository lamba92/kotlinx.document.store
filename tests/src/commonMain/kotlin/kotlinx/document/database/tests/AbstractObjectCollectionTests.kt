...
abstract class AbstractObjectCollectionTests(store: DataStore) : BaseTest(store) {
    ...
    fun `Fails if collection type is not serializable`() = runDatabaseTest {
    ...
    fun `Fails if collection type is primitive`() = runDatabaseTest {
    ...
    fun `Fails if collection type is array-like`() = runDatabaseTest {
